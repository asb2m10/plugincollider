...
class ScVarExtractor:
    ...
    def rewrite(self):
        ...
        token = pygments.lex(self.originalCode, self.lexer)
        tokens = list(token)
    ...
    def render(self):
        ...
        return template.format(synthName="sc_%03d" % self.idx, internalDef=code, arguments=arguments)
...
out = open("compiled.sc", "w")
for i in open("tweets_collection.sc").readlines() :
    i = i.strip()
    if i == "" :
        continue
